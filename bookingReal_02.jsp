...
<table>
	<tr>
		...
		<a href="bookingReal_02.jsp?year=<%=prevYear%>&month=<%=prevMonth%>">prev</a> 
		<%=year%>년 <%=month+1%>월 
		<a href="bookingReal_02.jsp?year=<%=nextYear%>&month=<%=nextMonth%>">next</a>		
		</td>
	</tr>
	<tr>
			...
			if((dateCOM == 1 || dateCOM == 0) && dateCOM_2 == -1){
				for(int i = 1; i <= 3; i++){
					...
					if(x == 0){
						...
						%><a href="bookingForm.jsp?year=<%=cal.get(Calendar.YEAR) %>&month=<%=cal.get(Calendar.MONTH)+1 %>&day=<%=cal.get(Calendar.DATE) %>&room_num=<%=i %>">예약가능</a><br /><%
					}else if(x == 2){
						%>예약완료<br /><%
					}else if(x == 3){
						%>예약진행중<br /><%
